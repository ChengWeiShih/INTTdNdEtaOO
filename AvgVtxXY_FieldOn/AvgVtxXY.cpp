...
AvgVtxXY::AvgVtxXY(
    int process_id_in,
    int number_of_file_in,
    int runnumber_in,
    int run_nEvents_in,
    std::string input_directory_in,
    std::string input_file_name_in, // note : prefix
    ...
    number_of_file(number_of_file_in),
    ...
    LineFill_Hist_Threshold(0.9),                          // todo:
    ...
    TH2D_threshold_advanced_2_CheckNBins(7)                // todo:
{
    ...
    gausScan = new TF1("gausScan", "gaus", Hist2D_DCA_min, Hist2D_DCA_max);
    c1 = new TCanvas("c1", "c1", 800, 600);

    h1D_slice_bestfit_height = new TH1D("h1D_slice_bestfit_height","h1D_slice_bestfit_height;Best fit Gaus amplitude;Entries",100, 0, 2000);
    h1D_slice_bestfit_sigma = new TH1D("h1D_slice_bestfit_sigma","h1D_slice_bestfit_sigma;Best fit Gaus width;Entries",100,0,0.01);
    h1D_slice_bestfit_3sigma_reduced_chi2 = new TH1D("h1D_slice_bestfit_3sigma_reduced_chi2","h1D_slice_bestfit_3sigma_reduced_chi2;3-sigma reduced chi2;Entries",500,0,200);
    // h1D_slice_bestfit_3sigma_reduced_chi2 = new TH1D("h1D_slice_bestfit_3sigma_reduced_chi2","h1D_slice_bestfit_3sigma_reduced_chi2;3-sigma reduced chi2;Entries",1000,0,50000);
    h1D_OneTrial_NGoodPoint = new TH1D("h1D_OneTrial_NGoodPoint","h1D_OneTrial_NGoodPoint;N good points;Entries",200,0,200);

    random_gen = new TRandom3(0);
    fitSine = new TF1("fitSine", "[0]*sin(x + [1]) + [2]", -TMath::Pi(), TMath::Pi());
    fitSine -> SetLineColor(kGreen+2);
    tge_random_sample = new TGraph();
...
void AvgVtxXY::ReadRootFile()
{
    ...
    tree_in = new TChain(input_tree_name.c_str());
    ...
    for (int i = 0; i < number_of_file; i++){
        int file_index = i + process_id * number_of_file;
        std::string file_to_add = (runnumber != -1) ? Form("%s/%s_%05d.root", input_directory.c_str(), input_file_name.c_str(), file_index) : Form("%s/%s_%06d.root", input_directory.c_str(), input_file_name.c_str(), file_index);

        if(std::filesystem::exists(file_to_add) == false){
            std::cout << "Error : the file to add does not exist : " << file_to_add << std::endl;
            exit(1);
        }

        std::cout<<"Adding file : "<<file_to_add<<std::endl;
        tree_in -> Add(file_to_add.c_str());
    }
...
void AvgVtxXY::SetUpHistograms(int iteration, int quadrant)
{
    ...
    // note : TGraph
    // note : inner
    insert_check = tge_map.insert( std::make_pair(
        Form("tge_%i_%i_InnerPhi_DeltaPhi", iteration, quadrant), 
        new TGraph()
    ) ).second; if (insert_check == false) {std::cout<<"Error : tge_map insert failed"<<std::endl; exit(1);}
    tge_map[Form("tge_%i_%i_InnerPhi_DeltaPhi", iteration, quadrant)] -> SetName(Form("tge_%i_%i_InnerPhi_DeltaPhi", iteration, quadrant));

    insert_check = tge_map.insert( std::make_pair(
        Form("tge_%i_%i_InnerPhi_DeltaPhi_RANSACrm", iteration, quadrant), 
        new TGraph()
    ) ).second; if (insert_check == false) {std::cout<<"Error : tge_map insert failed"<<std::endl; exit(1);}
    tge_map[Form("tge_%i_%i_InnerPhi_DeltaPhi_RANSACrm", iteration, quadrant)] -> SetName(Form("tge_%i_%i_InnerPhi_DeltaPhi_RANSACrm", iteration, quadrant));

    insert_check = tge_map.insert( std::make_pair(
        Form("tge_%i_%i_InnerPhi_DCA", iteration, quadrant), 
        new TGraph()
    ) ).second; if (insert_check == false) {std::cout<<"Error : tge_map insert failed"<<std::endl; exit(1);}
    tge_map[Form("tge_%i_%i_InnerPhi_DCA", iteration, quadrant)] -> SetName(Form("tge_%i_%i_InnerPhi_DCA", iteration, quadrant));
...
std::pair<double,double> AvgVtxXY::RunSingleCandidate(int iteration, int quadrant, std::pair<double,double> assume_center)
{
    ...
    for (auto &pair : cluster_pair_vec){
        ...
        if (DeltaPhi >= -7e-3 && DeltaPhi <= 7e-3){
            N_pair_in_region += 1;
        }
    ...
    GrooveAnalysis(h2D_map[Form("h2_%i_%i_InnerPhi_DeltaPhi", iteration, quadrant)], tge_map[Form("tge_%i_%i_InnerPhi_DeltaPhi", iteration, quadrant)] );
    std::vector<double> RANSACAnalysis_result = RANSACAnalysis(
        tge_map[Form("tge_%i_%i_InnerPhi_DeltaPhi", iteration, quadrant)], 
        tge_map[Form("tge_%i_%i_InnerPhi_DeltaPhi_RANSACrm", iteration, quadrant)]
    );

    // GrooveAnalysis(h2D_map[Form("h2_%i_%i_InnerPhi_DCA", iteration, quadrant)], tge_map[Form("tge_%i_%i_InnerPhi_DCA", iteration, quadrant)] );
    ...
    tge_map[Form("tge_%i_%i_InnerPhi_DeltaPhi_RANSACrm", iteration, quadrant)] -> Fit(fit_innerphi_DeltaPhi, "NQ", "", -M_PI, M_PI);
    ...
    c1 -> Clear();
    c1 -> cd();
    h2D_map[Form("h2_%i_%i_InnerPhi_DeltaPhi", iteration, quadrant)] -> Draw("COLZ0");
    tge_map[Form("tge_%i_%i_InnerPhi_DeltaPhi", iteration, quadrant)] -> Draw("P same");
    tge_map[Form("tge_%i_%i_InnerPhi_DeltaPhi_RANSACrm", iteration, quadrant)] -> Draw("P same");
    fitSine->SetParameters(
        RANSACAnalysis_result[0],
        RANSACAnalysis_result[1],
        RANSACAnalysis_result[2]
    );
    fitSine->Draw("same");
    c1 -> Write(Form("c1_h2_%i_%i_InnerPhi_DeltaPhi", iteration, quadrant));
    c1 -> Clear();

    // h2D_map[Form("h2_%i_%i_InnerPhi_DCA", iteration, quadrant)] -> Draw("COLZ0");
    // tge_map[Form("tge_%i_%i_InnerPhi_DCA", iteration, quadrant)] -> Draw("P same");
    // c1 -> Write(Form("c1_h2_%i_%i_InnerPhi_DCA", iteration, quadrant));
    // c1 -> Clear();
...
void AvgVtxXY::FindVertexQuadrant(int nIteration, double search_Window_half, std::pair<double,double> set_center)
{
    ...
    {   
        ...
        // for (int quadrant_i = 0; quadrant_i < 1; quadrant_i++)
        {
            // note : innerphi_delta_phi_fiterror, innerphi_dca_fiterror
            ...
            else 
            {
                ...
                if (current_pair.first < small_pair.first)
...
void AvgVtxXY::EndRun()
{
    ...
    h1D_slice_bestfit_height -> Write();
    h1D_slice_bestfit_sigma -> Write();
    h1D_slice_bestfit_3sigma_reduced_chi2 -> Write();
    h1D_OneTrial_NGoodPoint -> Write();
...
void AvgVtxXY::GrooveAnalysis(TH2D *h2_input,
                             TGraph * grGroove,
                            //  std::vector<double> excludePhiRanges = {},
                             double minSigmafactor, bool verbose) {

  // Clone the input histogram to avoid modifying the original

  int nSlices = h2_input->GetNbinsX();
...
//   std::vector<double> xValues, yValues, xErrors, yErrors;
  ...
  if (verbose) {
    std::cout << "=========================================================="
              << std::endl;
    std::cout << "Groove Analysis: Processing " << nSlices << " X slices"
              << std::endl;
    std::cout << "=========================================================="
              << std::endl;
  }
  ...
  // Analysis parameters
  const double yMin = h2_input->GetYaxis()->GetXmin();
  const double yMax = h2_input->GetYaxis()->GetXmax();
  const double yrange = yMax - yMin;
  const double fitWindowSize = (yrange/2.) * 0.1; // ±0.005 rad -> 0.01 rad total
  const double scanStep = (yrange/2.) * 0.01;     // 90% overlap -> 0.001 rad step

  // Loop through all X slices
  for (int iSlice = 0; iSlice < nSlices; iSlice++) {

    if (verbose && (iSlice + 1) % 10 == 0) {
      std::cout << "Processed " << (iSlice + 1) << "/" << nSlices
                << " slices..." << std::endl;
    }

    // Get X bin center for this slice
    double xCenter = h2_input->GetXaxis()->GetBinCenter(iSlice + 1);

    // Project Y-axis for this X slice
    TH1D *hProj =
        h2_input->ProjectionY(Form("%s_hProj_slice%d", h2_input->GetName(), iSlice), iSlice + 1, iSlice + 1);

    // if (hProj->GetEntries() < 10) {
    //   delete hProj;
    //   continue;
    // }

    // Flip the distribution: Create inverted histogram
    // TH1D *hFlipped = (TH1D *)hProj->Clone(Form("hFlipped_slice%d", iSlice));
    double maxContent = hProj->GetBinContent(hProj->GetMaximumBin()) * 0.95;
    for (int iBin = 1; iBin <= hProj->GetNbinsX(); iBin++) {
      double originalContent = hProj->GetBinContent(iBin);
      double newContent = (maxContent - originalContent < 0) ? 0 : maxContent - originalContent; 

      hProj->SetBinContent(iBin, newContent);

    //   hFlipped->SetBinContent(iBin, maxContent - originalContent);
    }

    // Scanning fit approach
    double bestChi2NDF = 1e15;
    double bestMean = 0;
    double bestSigma = 0;
    double bestHeight = 0;

    std::vector<double> scanCenters; scanCenters.clear();
    for (double center = yMin + fitWindowSize / 2.0;
         center <= yMax - fitWindowSize / 2.0; center += scanStep) {
      scanCenters.push_back(center);
    }

    // Try each scan window
    for (size_t iScan = 0; iScan < scanCenters.size(); iScan++) {
      double scanCenter = scanCenters[iScan];
      double fitMin = scanCenter - fitWindowSize / 2.0;
      double fitMax = scanCenter + fitWindowSize / 2.0;

      // Find local maximum in this window
      double localMax = 0;
      double localMaxPos = scanCenter;
      for (int iBin = 1; iBin <= hProj->GetNbinsX(); iBin++) {
        double binCenter = hProj->GetBinCenter(iBin);
        if (binCenter >= fitMin && binCenter <= fitMax) {
          double content = hProj->GetBinContent(iBin);
          if (content > localMax) {
            localMax = content;
            // localMaxPos = binCenter;
          }
        }
      }
    ...
    //   TF1 *gausScan =
    //       new TF1(Form("gausScan_%d_%zu", iSlice, iScan), "gaus", yMin, yMax);
      gausScan->SetParameter(0, localMax);
      gausScan->SetParameter(1, localMaxPos);
      gausScan->SetParameter(2, scanStep * 3.);

      // Set parameter limits
      gausScan->SetParLimits(1, fitMin, fitMax); // Mean within window
      gausScan->SetParLimits(2, fitWindowSize * 0.01, fitWindowSize);   // Sigma constraint

      // Perform fit
      hProj->Fit(gausScan, "NQB", "", fitMin, fitMax);

      if (verbose) {
        std::cout << "  Slice " << iSlice << ", Scan " << iScan
                  << ": Center " << scanCenter <<", width: "<<fitWindowSize
                  << ", Local Max " << localMax << ", localMaxPos " << localMaxPos
                  << ": Fit window [" << fitMin << ", " << fitMax << "]"
                  << std::endl;
      }

      double fittedHeight = gausScan->GetParameter(0);
      double fittedMean = gausScan->GetParameter(1);
      double fittedSigma = gausScan->GetParameter(2);

      // Calculate chi2 in ±3σ range
      double chi2_3sigma_min = fittedMean - 3.0 * fittedSigma;
      double chi2_3sigma_max = fittedMean + 3.0 * fittedSigma;

      double chi2_sum = 0.0;
      int nPoints = 0;
      for (int iBin = 1; iBin <= hProj->GetNbinsX(); iBin++) {
        double binCenter = hProj->GetBinCenter(iBin);
        if (binCenter >= chi2_3sigma_min && binCenter <= chi2_3sigma_max) {
          double observed = hProj->GetBinContent(iBin);
          double expected = gausScan->Eval(binCenter);
          double error = hProj->GetBinError(iBin);
          if (error > 0) {
            // chi2_sum += std::pow((observed - expected) / 1, 2);
            chi2_sum += std::pow((observed - expected) / error, 2);
            nPoints++;
          }
        }
      }

      int ndf = nPoints - 3; // 3 parameters in Gaussian
      double chi2NDF = (ndf > 0) ? (chi2_sum / ndf) : 1e15;
    ...
    //     c1 -> cd();
    //     hProj -> Draw("hist");
    //     gausScan -> SetLineColor(kRed);
    //     gausScan -> SetNpx(5000);
    //     gausScan -> Draw("same");
    //     c1 -> Write(Form("GrooveFit_Slice%d_Scan%d", iSlice, iScan));
    //     std::cout<<"fit height: "<<gausScan -> GetParameter(0)<<" mean: "<<gausScan -> GetParameter(1)<<" sigma: "<<gausScan -> GetParameter(2)<<" chi2NDF: "<<gausScan->GetChisquare()/gausScan->GetNDF()<<", 3sigma: "<<chi2NDF<<std::endl;
    //   }
      ...
      if (chi2NDF < bestChi2NDF) {
        bestChi2NDF = chi2NDF;
        bestMean = fittedMean;
        bestSigma = fittedSigma;
        bestHeight = fittedHeight;
      }

    //   delete gausScan;
    }

    std::cout<<"Best fit for slice " << iSlice << ": Height " << bestHeight
              << ", Mean " << bestMean << ", Sigma " << bestSigma
              << ", 3-sigma reduced Chi2 " << bestChi2NDF << std::endl;

    h1D_slice_bestfit_height -> Fill(bestHeight);
    h1D_slice_bestfit_sigma -> Fill(bestSigma);
    h1D_slice_bestfit_3sigma_reduced_chi2 -> Fill(bestChi2NDF);

    // Apply filtering criteria
    bool excludeThisPoint = false;
    double phi = h2_input->GetXaxis()->GetBinCenter(iSlice + 1);

    if ((phi >= -1.6 && phi <= -1.0)  || 
        (phi >= -0.15 && phi <= 0.15) ||
        (phi >= 1.55 && phi <= 1.9)) {
      excludeThisPoint = true;
    }

    // Filter 2: Check sigma threshold
    // if (bestSigma < 0.002 || bestSigma > 0.0036) {
    if (bestSigma < 0.002 || bestSigma > 0.005) {
      excludeThisPoint = true;
    }
    ...
    // Add point if not excluded
    if (!excludeThisPoint) {
    //   xValues.push_back(xCenter);
    //   yValues.push_back(bestMean);
    //   xErrors.push_back(0.0);
    //   yErrors.push_back(bestSigma);

      grGroove->SetPoint(grGroove->GetN(), xCenter, bestMean);
    //   grGroove->SetPointError(grGroove->GetN() - 1, 0.0, bestSigma);
    }
...
//     grGroove->SetPoint(i, xValues[i], yValues[i]);
//     grGroove->SetPointError(i, xErrors[i], yErrors[i]);
//   }

  h1D_OneTrial_NGoodPoint -> Fill(grGroove->GetN());   
  grGroove->SetMarkerStyle(20);
  grGroove->SetMarkerSize(0.8);
  grGroove->SetMarkerColor(kRed);
  grGroove->SetLineColor(kRed);

  if (verbose) {
    std::cout << "=========================================================="
              << std::endl;
    std::cout << "Analysis complete!" << std::endl;
    std::cout << "Points added: " << grGroove->GetN() << std::endl;
    std::cout << "Points excluded: " << (nSlices - grGroove->GetN()) << std::endl;
    std::cout << "=========================================================="
              << std::endl;
  }
...
std::vector<double> AvgVtxXY::RANSACAnalysis(TGraph * gr_input, TGraph * grRANSAC, Double_t inlier_threshold, Int_t n_iterations, double sample_ratio, bool verbose){

    int n_points = gr_input -> GetN();
    int n_sample = int(n_points * sample_ratio);
    std::vector<int> selected_point; selected_point.clear();

    Double_t best_A = 0, best_phi = 0, best_C = 0;
    double best_residual_stddev = 0;
    Int_t best_inlier_count = 0;
    double best_rchi2  = 1e9;

    for (int i = 0; i < n_iterations; i++){
        selected_point.clear();
        tge_random_sample->Set(0);

        while (selected_point.size() < n_sample){ // note : stop when selected_point size reach n_sample
            int rand_index = random_gen -> Integer(n_points); // note : 0 to n_points-1
            if (std::find(selected_point.begin(), selected_point.end(), rand_index) == selected_point.end()){
                selected_point.push_back(rand_index);
                
                tge_random_sample -> SetPoint(tge_random_sample -> GetN(), 
                    gr_input -> GetX()[rand_index],
                    gr_input -> GetY()[rand_index]
                );
            }
        } 

        // note : [0]*sin(x + [1]) + [2]
        fitSine -> SetParameters(0.01, 0, 0.001);
        tge_random_sample -> Fit(fitSine, "NQ", "", -TMath::Pi(), TMath::Pi());

        std::vector<double> residuals; residuals.clear();
        for (int j = 0; j < tge_random_sample->GetN(); j++){
            Double_t x_val = tge_random_sample -> GetX()[j];
            Double_t y_val = tge_random_sample -> GetY()[j];
            Double_t y_fit = fitSine -> Eval(x_val);
            residuals.push_back(y_val - y_fit);
        }
        double residuals_stddev = AvgVtxXY::vector_stddev(residuals); // note: based on 50 points only

        // int inlier_count = 0;
        // for (int j = 0; j < n_points; j++){
        //     Double_t x_val = gr_input -> GetX()[j];
        //     Double_t y_val = gr_input -> GetY()[j];
        //     Double_t y_fit = fitSine -> Eval(x_val);
        //     if (fabs(y_val - y_fit) < inlier_threshold * residuals_stddev){
        //         inlier_count += 1;
        //     }
        // }

        // if (inlier_count > best_inlier_count){
        if (fitSine -> GetChisquare() / fitSine -> GetNDF() < best_rchi2){
            best_rchi2 = fitSine -> GetChisquare() / fitSine -> GetNDF();
            // best_inlier_count = inlier_count;
            best_A = fitSine -> GetParameter(0);
            best_phi = fitSine -> GetParameter(1);
            best_C = fitSine -> GetParameter(2);
            best_residual_stddev = residuals_stddev;

            best_inlier_count = 0;
            grRANSAC -> Set(0);
            for (int j = 0; j < gr_input->GetN(); j++){
                Double_t x_val = gr_input -> GetX()[j];
                Double_t y_val = gr_input -> GetY()[j];
                Double_t y_fit = fitSine -> Eval(x_val);
                if (fabs(y_val - y_fit) < inlier_threshold * residuals_stddev){
                    grRANSAC -> SetPoint(grRANSAC -> GetN(), x_val, y_val);
                    best_inlier_count += 1;
                }
            }
        }
    ...
    std::cout<<"RANSAC best fit: A = "<<best_A<<", phi = "<<best_phi<<", C = "<<best_C<<", inlier count = "<<best_inlier_count<<"/"<<gr_input->GetN()<<std::endl;

    grRANSAC->SetMarkerStyle(20);
    grRANSAC->SetMarkerSize(0.8);
    grRANSAC->SetMarkerColor(kBlue);
    grRANSAC->SetLineColor(kBlue);

    return {best_A, best_phi, best_C, double(best_inlier_count), best_residual_stddev};
}