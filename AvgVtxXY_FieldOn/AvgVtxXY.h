...
class AvgVtxXY {
    ...
    protected:
        ...
        std::map<std::string, TGraph*> tge_map; // note : for each iteration
        ...
        // note : for the field ON
        TF1 * gausScan;
        void GrooveAnalysis(TH2D *h2_input, TGraph * grGroove, double minSigmafactor = 0.15, bool verbose = false);
        TCanvas * c1;
        TH1D * h1D_slice_bestfit_height;
        TH1D * h1D_slice_bestfit_sigma;
        TH1D * h1D_slice_bestfit_3sigma_reduced_chi2;
        TH1D * h1D_OneTrial_NGoodPoint;

        TRandom3 * random_gen;
        TF1 * fitSine;
        TGraph * tge_random_sample;
        std::vector<double> RANSACAnalysis(TGraph * gr_input, TGraph * grRANSAC, Double_t inlier_threshold = 4.2,
                             Int_t n_iterations = 1000, double sample_ratio = 0.9,
                             bool verbose = true);

        double  vector_average (std::vector <double> input_vector) {
            return std::accumulate( input_vector.begin(), input_vector.end(), 0.0 ) / double(input_vector.size());
        }
        ...
        const double Hist2D_DeltaPhi_min = -0.1; // note : -0.03;
        const double Hist2D_DeltaPhi_max =  0.1; // note :  0.03;

        const double Hist2D_DCA_min = -4.0;
        const double Hist2D_DCA_max =  4.0;
