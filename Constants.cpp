...
namespace Constants{
    ...
    // note : vtxZQA
    std::pair<double, double> cut_vtxZDiff = {-13, 12}; // {-3.5, 4.5}; // note : MBDz - INTTz
    std::pair<double, double> cut_TrapezoidalFitWidth = {1.5, 10}; // {2, 7}; // {1.5, 5.5};
    std::pair<double, double> cut_TrapezoidalFWHM = {1.2, 10}; // {2.5, 7.8}; // {2,8};
    std::pair<double, double> cut_INTTvtxZError = {-10000, 2};

    // note : for analysis
    std::pair<double, double> cut_GlobalMBDvtxZ = {-60, 60};
    std::pair<double, double> cut_AnaVtxZ = {-10, 10}; // note : used by vtxZDist.cpp

    int cut_InttBcoFullDIff_next = 80;

    int Semi_inclusive_bin = 13;
    int Semi_inclusive_interval = 70; // note : 70, for example

    int HighNClus = 150; // Division: it was 500 for AuAu
    ...
    double cut_GoodRecoVtxZ = 3.; // note : cm // note : used by vtxZDist.cpp
    ...
    std::vector<ULong_t> selected_bunch_id = {1, 9, 17, 25, 33, 41, 49, 57, 65, 73, 81, 89, 97, 105};
