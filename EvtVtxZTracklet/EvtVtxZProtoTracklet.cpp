...
void EvtVtxZProtoTracklet::GetINTTvtxZ()
{
              ...
              for (int outer_phi_clu_i = 0; outer_phi_clu_i < int(outer_clu_phi_map_PostCut[true_scan_i].size()); outer_phi_clu_i++)
              {
                  ...
                  // note : field ON
                    ...
                    if (
                        ...
                        (delta_phi_correct <= DeltaPhiCutInDegree.first.first || delta_phi_correct >= DeltaPhiCutInDegree.first.second)
                       ) 
                       {continue;}
                  }
                    ...
                    if (!IsFieldOn){ // note : field off
                      ...
                      if (
                        ...
                        //   (DCA_sign > DCAcutIncm.second.first && DCA_sign < DCAcutIncm.second.second) == false
                            (DCA_sign <= DCAcutIncm.first.first || DCA_sign >= DCAcutIncm.first.second)
                       ) 
                       {continue;}
                    }
                  }
  ...
  gaus_fit -> SetParameters(line_breakdown_hist_max_content, line_breakdown_hist_max_center, temp_fit_region_half, 0);
  ...
  double width_fit_range_l = line_breakdown_hist_max_center - temp_fit_region_half * 0.65;
  double width_fit_range_r = line_breakdown_hist_max_center + temp_fit_region_half * 0.65;
  ...
  {
      gaus_fit_vec[fit_i] -> SetParameters(line_breakdown_hist_max_content, line_breakdown_hist_max_center, temp_fit_region_half, 0);
    ...
    //   double mean_fit_range_r = line_breakdown_hist_max_center + Half_N_group_half_width * N_width_ratio;
      ...
      double mean_fit_range_l = line_breakdown_hist_max_center - temp_fit_region_half * N_width_ratio;
      double mean_fit_range_r = line_breakdown_hist_max_center + temp_fit_region_half * N_width_ratio;
    ...
    )
  {
    ...
    line_breakdown_hist_zoomin -> GetXaxis() -> SetRangeUser(
        ...
        // gaus_fit_vec.front() -> GetParameter(1) + Half_N_group_half_width
        gaus_fit_vec.front() -> GetParameter(1) - temp_fit_region_half,
        gaus_fit_vec.front() -> GetParameter(1) + temp_fit_region_half
    );
...
void EvtVtxZProtoTracklet::FillRecoINTTVtxZH1D(int event_index)
{
    ...
    if (event_index % 20 == 0)
    {
        ...
        std::cout<<"In Filling RecoZ: "<<event_index<<", temp_INTTvtxZ: "<<temp_INTTvtxZ<<std::endl;
    }
...
std::vector<double> EvtVtxZProtoTracklet::find_Ngroup(TH1D * hist_in)
{
    ...
    for (int i = 0; i < hist_in -> GetNbinsX(); i++){
        ...
        // note : Oct 23, 2025, change /2. to 0.8, for the field-on data, due to high background
