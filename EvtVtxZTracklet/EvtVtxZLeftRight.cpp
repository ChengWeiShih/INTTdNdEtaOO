...
EvtVtxZLeftRight::EvtVtxZLeftRight(
    int process_id_in,
    int runnumber_in,
    int run_nEvents_in,
...
void EvtVtxZLeftRight::PrepareOutPutFileName()
{
    ...
    output_filename = "EvtVtxZLeftRight";
    ...
    output_filename += (isLeftOnly) ? "_LeftOnly" : "_LeftOnly_RightOnly";
...
void EvtVtxZLeftRight::SetGeoOffset(std::map<std::string, std::vector<double>> input_geo_offset_map)
{
    std::cout<<"In EvtVtxZLeftRight, reading geo offset map:"<<std::endl;
...
double EvtVtxZLeftRight::CheckGeoOffsetMap()
{
    double sum = 0;
    for (auto &pair : geo_offset_map)
    {
...
std::map<std::string, int> EvtVtxZLeftRight::GetInputTreeBranches(TTree * m_tree_in)
{
...
void EvtVtxZLeftRight::PrepareRootFile()
{
    ...
    if (RunVtxZReco){
        std::string left_or_right = (isLeftOnly) ? "Left" : "Right";

        b_INTTvtxZ            = tree_out -> Branch(Form("%s_INTTvtxZ",left_or_right.c_str()), &out_INTTvtxZ);
        b_INTTvtxZError       = tree_out -> Branch(Form("%s_INTTvtxZError",left_or_right.c_str()), &out_INTTvtxZError);
        b_NgroupTrapezoidal   = tree_out -> Branch(Form("%s_NgroupTrapezoidal",left_or_right.c_str()), &out_NgroupTrapezoidal);
        b_NgroupCoarse        = tree_out -> Branch(Form("%s_NgroupCoarse",left_or_right.c_str()), &out_NgroupCoarse);
        b_TrapezoidalFitWidth = tree_out -> Branch(Form("%s_TrapezoidalFitWidth",left_or_right.c_str()), &out_TrapezoidalFitWidth);
        b_TrapezoidalFWHM     = tree_out -> Branch(Form("%s_TrapezoidalFWHM",left_or_right.c_str()), &out_TrapezoidalFWHM);

        // b_ClusEta_INTTz       = tree_out -> Branch(Form("%s_ClusEta_INTTz",left_or_right.c_str()), &out_ClusEta_INTTz);
    }

}

// void EvtVtxZLeftRight::GetTriggerInfo()
// {
...
void EvtVtxZLeftRight::PrepareINTTvtxZ()
{
    ...
    std::string left_or_right = (isLeftOnly) ? "Left" : "Right";
    h2D_SelectedINTTXY = new TH2D(Form("h2D_%s_SelectedINTTXY",left_or_right.c_str()), Form("h2D_%s_SelectedINTTXY;ClusX [cm];ClusY [cm]",left_or_right.c_str()), 300, -15, 15, 300, -15, 15);
}

void EvtVtxZLeftRight::PrepareClusterVec()
{
    for (int clu_i = 0; clu_i < ClusX -> size(); clu_i++)
    {
        EvtVtxZLeftRight::clu_info this_clu;
        ...
        if (
            isLeftOnly && 
            std::find(ladder_left_vec.begin(), ladder_left_vec.end(), Form("%i_%i", this_clu.layerID, this_clu.ladderPhiID)) == ladder_left_vec.end()
        ) {continue;}

        if (
            isLeftOnly == false && // note : focus on the right side 
            std::find(ladder_left_vec.begin(), ladder_left_vec.end(), Form("%i_%i", this_clu.layerID, this_clu.ladderPhiID)) != ladder_left_vec.end()
        ) {continue;}

        std::vector<EvtVtxZLeftRight::clu_info>* p_evt_sPH_nocolumn_vec =
        ...
        h2D_SelectedINTTXY -> Fill(this_clu.x, this_clu.y);

        std::vector<EvtVtxZLeftRight::clu_info>* p_evt_sPH_nocolumn_vec_PostCut =
...
void EvtVtxZLeftRight::GetINTTvtxZ()
{

    inner_clu_phi_map_PostCut.clear();
    outer_clu_phi_map_PostCut.clear();
    inner_clu_phi_map_PostCut = std::vector<std::vector<std::pair<bool,EvtVtxZLeftRight::clu_info>>>(360);
    outer_clu_phi_map_PostCut = std::vector<std::vector<std::pair<bool,EvtVtxZLeftRight::clu_info>>>(360);
...
std::pair<double,double> EvtVtxZLeftRight::rotatePoint(double x, double y)
{
...
std::vector<EvtVtxZLeftRight::clu_info> EvtVtxZLeftRight::GetRoatedClusterVec(std::vector<EvtVtxZLeftRight::clu_info> input_cluster_vec)
{
    std::vector<EvtVtxZLeftRight::clu_info> output_cluster_vec; output_cluster_vec.clear();

    for (EvtVtxZLeftRight::clu_info this_clu : input_cluster_vec)
    {
        std::pair<double,double> rotated_xy = rotatePoint(this_clu.x, this_clu.y);
     
        EvtVtxZLeftRight::clu_info rotated_clu = this_clu;
...
// void EvtVtxZLeftRight::GetClusUpdated()
// {
...
// void EvtVtxZLeftRight::GetTrackletPair(std::vector<pair_str> &input_TrackletPair_vec, bool isRotated)
// {

//     inner_clu_phi_map.clear();
//     outer_clu_phi_map.clear();
//     inner_clu_phi_map = std::vector<std::vector<std::pair<bool,EvtVtxZLeftRight::clu_info>>>(360);
//     outer_clu_phi_map = std::vector<std::vector<std::pair<bool,EvtVtxZLeftRight::clu_info>>>(360);

//     if (temp_INTTvtxZ != temp_INTTvtxZ || temp_INTTvtxZError != temp_INTTvtxZError) {return;}

//     std::vector<EvtVtxZLeftRight::clu_info> temp_evt_sPH_inner_nocolumn_vec = (isRotated) ? GetRoatedClusterVec(evt_sPH_inner_nocolumn_vec) : evt_sPH_inner_nocolumn_vec;
...
void EvtVtxZLeftRight::FillRecoINTTVtxZH1D(int event_index)
{
...
void EvtVtxZLeftRight::MainProcess()
{
    ...
    for (int i = 0; i < run_nEvents; i++){
        ...
        if (i % 20 == 0) {
            std::cout<<"In EvtVtxZLeftRight, Processing event: "<<i<<std::endl;
        }
...
void EvtVtxZLeftRight::EvtCleanUp()
{
...
void EvtVtxZLeftRight::EndRun(int close_file_in)
{   
    file_out -> cd();
    tree_out -> Write();
    h2D_SelectedINTTXY -> Write();
    // reco_INTTvtxZ -> Write();
    file_out -> Close();
...
double EvtVtxZLeftRight::get_radius(double x, double y)
{
...
double EvtVtxZLeftRight::get_delta_phi(double angle_1, double angle_2)
{
...
double EvtVtxZLeftRight::calculateAngleBetweenVectors(double x1, double y1, double x2, double y2, double targetX, double targetY) {
...
double EvtVtxZLeftRight::Get_extrapolation(double given_y, double p0x, double p0y, double p1x, double p1y) // note : x : z, y : r
{
...
std::pair<double,double> EvtVtxZLeftRight::Get_possible_zvtx(double rvtx, std::vector<double> p0, std::vector<double> p1) // note : inner p0, outer p1, vector {r,z, zid}, -> {y,x}
{
...
void EvtVtxZLeftRight::line_breakdown(TH1D* hist_in, std::pair<double,double> line_range)
{ 
...
void EvtVtxZLeftRight::trapezoidal_line_breakdown(TH1D * hist_in, double inner_r, double inner_z, int inner_zid, double outer_r, double outer_z, int outer_zid)
{
...
std::vector<double> EvtVtxZLeftRight::find_Ngroup(TH1D * hist_in)
{
...
double  EvtVtxZLeftRight::vector_average (std::vector <double> input_vector) {
	return accumulate( input_vector.begin(), input_vector.end(), 0.0 ) / double(input_vector.size());
}

double EvtVtxZLeftRight::vector_stddev (std::vector <double> input_vector){
...
double EvtVtxZLeftRight::get_clu_eta(std::vector<double> vertex, std::vector<double> clu_pos)
{
...
// note : p1 : inner layer
// note : p2 : outer layer
std::pair<double,double> EvtVtxZLeftRight::Get_eta(std::vector<double>p0, std::vector<double>p1, std::vector<double>p2)
{
    // if (track_gr -> GetN() != 0) {cout<<"In EvtVtxZLeftRight class, track_gr is not empty, track_gr size : "<<track_gr -> GetN()<<endl; exit(0);}
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        // cout<<"In EvtVtxZLeftRight class, point : "<<i<<" r : "<<r_vec[i]<<" z : "<<z_vec[i]<<" re : "<<re_vec[i]<<" ze : "<<ze_vec[i]<<endl;
    }    
...
double EvtVtxZLeftRight::grEY_stddev(TGraphErrors * input_grr)
{
...
std::pair<double, double> EvtVtxZLeftRight::mirrorPolynomial(double a, double b) {
    // Interchange 'x' and 'y'
...
void EvtVtxZLeftRight::Characterize_Pad (TPad *pad, float left, float right, float top, float bottom, bool set_logY, int setgrid_bool)
{
...
void EvtVtxZLeftRight::PrepareEvtCanvas()
{
