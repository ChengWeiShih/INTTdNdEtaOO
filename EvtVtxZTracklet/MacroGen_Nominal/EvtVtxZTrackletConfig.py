"""
EvtVtxZTrackletConfig  –  dataclass defining all parameters for an EvtVtxZTracklet pass.
...
PairPairDouble = Tuple[Tuple[float, float], Tuple[float, float]]

_FRAMEWORK_DIR = os.path.dirname(os.path.abspath(__file__))

@dataclass
class EvtVtxZTrackletConfig:
    # ── Pass identity ──────────────────────────────────────────────────────────
    ...
    input_directory: str = ""
    input_file_name: str = "Ntuple_{run_num:08d}_standalone_{process:05d}.root"
    ...
    vertexXYIncm: PairDouble = (-0.053125, 0.13593800)
    IsFieldOn: bool = True
    ...
    """Directory containing Run_EvtVtxZTracklet_template.C and run_job_template.sh."""
