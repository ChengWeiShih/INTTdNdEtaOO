...
void Run_vtxZDist(
  ...
  bool Apply_cut = true, // note: vtxZQA cut, don't touch
  bool ApplyVtxZReWeighting = false, // note : don't touch, regardless data or MC
  std::pair<bool, int> ApplyEvtBcoFullDiffCut = {true, 80}, // note : don't touch, regardless data or MC
  bool isMBDAsymmCut = false,
  bool CheckDataDrivenVtxZResolution = true,

  bool IsRunNoVtxZCut = true
)
{
  ...
  if (IsRunNoVtxZCut) // note : no vtxZ QA
  {
