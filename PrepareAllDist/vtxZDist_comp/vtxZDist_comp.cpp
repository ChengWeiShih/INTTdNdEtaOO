...
void vtxZDist_comp::PrepareINTTvtxZReWeight()
{
    ...
    TH1D * data_reference_Inclusive70_tight20 = (TH1D*) data_h1D_map_map[data_input_directory_pair_vec[0].first]["h1D_INTTz_Inclusive70"] -> Clone("h1D_INTTz_Inclusive70_tight20");
    data_reference_Inclusive70_tight20 -> Sumw2(true);

    TH1D * data_reference_Inclusive100_tight20 = (TH1D*) data_h1D_map_map[data_input_directory_pair_vec[0].first]["h1D_INTTz_Inclusive100"] -> Clone("h1D_INTTz_Inclusive100_tight20");
    data_reference_Inclusive100_tight20 -> Sumw2(true);


    for (int bin = 1; bin <= data_reference_Inclusive70_tight20 -> GetNbinsX(); bin++){

        if (data_reference_Inclusive70_tight20 -> GetBinCenter(bin) < -20 || data_reference_Inclusive70_tight20 -> GetBinCenter(bin) > 20) {
            data_reference_Inclusive70_tight20 -> SetBinContent(bin, 0);
            data_reference_Inclusive100_tight20 -> SetBinContent(bin, 0);
        }
    }

    data_reference_Inclusive70_tight20 -> Scale(1.0 / data_reference_Inclusive70_tight20 -> Integral());
    data_reference_Inclusive100_tight20 -> Scale(1.0 / data_reference_Inclusive100_tight20 -> Integral());
    ...
    for (auto &pair : MC_h1D_map_map){
        ...
        // Division ---for tight 20 cm-----------------------------------------------------------------------------------------------------------------------------

        std::cout<<"AAAA"<<std::endl;
        reweighting_ratio.insert(
            std::make_pair(
                Form("%d_MISC_SimZvtx_Inclusive70_tight20", count),
                (TH1D*) pair.second["h1D_INTTz_Inclusive70"]  -> Clone( Form("%d_%s_MISC_SimZvtx_Inclusive70_tight20",count,MC_input_directory_map[pair.first].second.c_str()))
            )
        );

        reweighting_ratio.insert(
            std::make_pair(
                Form("%d_MISC_SimZvtx_Inclusive100_tight20", count),
                (TH1D*) pair.second["h1D_INTTz_Inclusive100"] -> Clone( Form("%d_%s_MISC_SimZvtx_Inclusive100_tight20",count,MC_input_directory_map[pair.first].second.c_str()))
            )
        );

        // reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive70_tight20", count)]  = (TH1D*) pair.second["h1D_INTTz_Inclusive70_tight20"]  -> Clone( Form("%d_%s_MISC_SimZvtx_Inclusive70_tight20",count,MC_input_directory_map[pair.first].second.c_str()));
        // reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive100_tight20", count)] = (TH1D*) pair.second["h1D_INTTz_Inclusive100_tight20"] -> Clone( Form("%d_%s_MISC_SimZvtx_Inclusive100_tight20",count,MC_input_directory_map[pair.first].second.c_str()));
        std::cout<<"bbbbb"<<std::endl;
        for (int bin = 1; bin <= reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive70_tight20", count)] -> GetNbinsX(); bin++){
            
            if (reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive70_tight20", count)] -> GetBinCenter(bin) < -20 || reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive70_tight20", count)] -> GetBinCenter(bin) > 20){
                    reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive70_tight20", count)] -> SetBinContent(bin, 0);
                    reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive100_tight20", count)] -> SetBinContent(bin, 0);
            }            
        }
        reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive70_tight20", count)] -> Scale(1.0 / reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive70_tight20", count)] -> Integral());
        reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive100_tight20", count)] -> Scale(1.0 / reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive100_tight20", count)] -> Integral());
        std::cout<<"ccccc"<<std::endl;

        reweighting_ratio.insert(
            std::make_pair(
                Form("%d_Inclusive70_tight20", count),
                (TH1D*) pair.second["h1D_INTTz_Inclusive70"] -> Clone((MC_input_directory_map[pair.first].second + "_Inclusive70_tight20").c_str())
            )
        );
        std::cout<<"ddddd"<<std::endl;

        reweighting_ratio[Form("%d_Inclusive70_tight20", count)] -> Sumw2(true);
        reweighting_ratio[Form("%d_Inclusive70_tight20", count)] -> Divide(data_reference_Inclusive70_tight20, reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive70_tight20", count)], 1, 1);
        reweighting_ratio[Form("%d_Inclusive70_tight20", count)] -> GetYaxis() -> SetTitle("Data/MC");
        reweighting_ratio[Form("%d_Inclusive70_tight20", count)] -> Write((MC_input_directory_map[pair.first].second + "_Inclusive70_tight20").c_str());
        std::cout<<"eeee"<<std::endl;

        reweighting_ratio.insert(
            std::make_pair(
                Form("%d_Inclusive100_tight20", count),
                (TH1D*) pair.second["h1D_INTTz_Inclusive100"] -> Clone((MC_input_directory_map[pair.first].second + "_Inclusive100_tight20").c_str())
            )
        );
        std::cout<<"ffff"<<std::endl;

        reweighting_ratio[Form("%d_Inclusive100_tight20", count)] -> Sumw2(true);
        reweighting_ratio[Form("%d_Inclusive100_tight20", count)] -> Divide(data_reference_Inclusive100_tight20, reweighting_ratio[Form("%d_MISC_SimZvtx_Inclusive100_tight20", count)], 1, 1);
        reweighting_ratio[Form("%d_Inclusive100_tight20", count)] -> GetYaxis() -> SetTitle("Data/MC");
        reweighting_ratio[Form("%d_Inclusive100_tight20", count)] -> Write((MC_input_directory_map[pair.first].second + "_Inclusive100_tight20").c_str());
