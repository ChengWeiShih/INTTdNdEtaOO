...
RestDist::RestDist(
    ...
    isCheckBestPair(isCheckBestPair_in)
{
    ...
    temp_all_pair_DeltaPhi_vec.clear();
...
void RestDist::PrepareOutPutFileName()
{
    ...
    output_filename += (isCheckBestPair) ? "_CheckBestPair" : "";
...
void RestDist::PrepareHist()
{
    ...
    h1D_map.insert(
        std::make_pair(
            ...
            new TH1D("h1D_PairDeltaPhi_Narrow","h1D_PairDeltaPhi_Narrow;Pair #Delta#phi;Entries",100,-0.1,0.1)
        )
    );
    ...
    h1D_map.insert(std::make_pair("h1D_NClus", new TH1D("h1D_NClus", "h1D_NClus;NClus;Entries (/80)",100,0,2500)));
    h1D_map.insert(std::make_pair("h1D_NClus_inner", new TH1D("h1D_NClus_inner", "h1D_NClus_inner;NClus (inner);Entries (/45)",100,0,2000)));
    h1D_map.insert(std::make_pair("h1D_NClus_outer", new TH1D("h1D_NClus_outer", "h1D_NClus_outer;NClus (outer);Entries (/45)",100,0,2000)));
    
    h1D_map.insert(std::make_pair("h1D_inner_typeA_NClus", new TH1D("h1D_inner_typeA_NClus", "h1D_inner_typeA_NClus;NClus (inner, type A);Entries (/32)",100,0,1500)));
    h1D_map.insert(std::make_pair("h1D_outer_typeA_NClus", new TH1D("h1D_outer_typeA_NClus", "h1D_outer_typeA_NClus;NClus (outer, type A);Entries (/32)",100,0,1500)));
    h1D_map.insert(std::make_pair("h1D_inner_typeB_NClus", new TH1D("h1D_inner_typeB_NClus", "h1D_inner_typeB_NClus;NClus (inner, type B);Entries (/32)",100,0,1500)));
    h1D_map.insert(std::make_pair("h1D_outer_typeB_NClus", new TH1D("h1D_outer_typeB_NClus", "h1D_outer_typeB_NClus;NClus (outer, type B);Entries (/32)",100,0,1500)));
    ...
    h2D_map.insert(
        std::make_pair(
            ...
            new TH2D("h2D_confirm_InttNClus_MbdChargeSum","h2D_confirm_InttNClus_MbdChargeSum;INTT NClus;MBD charge sum", 200, 0, 2500, 200, 0, 1000)
        )
    );
    ...
    h2D_map.insert(
        std::make_pair(
            ...
            new TH2D("h2D_confirm_InttInnerOuterNClus","h2D_confirm_InttInnerOuterNClus;NClus (inner);NClus (outer)", 200, 0, 2500, 200, 0, 2500)
        )
    );
    ...
    h2D_map.insert(std::make_pair("h2D_MBDcharge_south_north",   new TH2D("h2D_MBDcharge_south_north","h2D_MBDcharge_south_north;MBD charge sum (north);MBD charge sum (south)",100,0,400,100,0,400)));
    h2D_map.insert(std::make_pair("h2D_MBDvtxZ_MBDchargeSum",   new TH2D("h2D_MBDvtxZ_MBDchargeSum","h2D_MBDvtxZ_MBDchargeSum;MBD vtxZ [cm];MBD charge sum (all)",120,-60,60,100,0,800)));
    h2D_map.insert(std::make_pair("h2D_MBDvtxZ_MBDchargeSouth", new TH2D("h2D_MBDvtxZ_MBDchargeSouth","h2D_MBDvtxZ_MBDchargeSouth;MBD vtxZ [cm];MBD charge sum (south)",120,-60,60,100,0,400)));
    h2D_map.insert(std::make_pair("h2D_MBDvtxZ_MBDchargeNorth", new TH2D("h2D_MBDvtxZ_MBDchargeNorth","h2D_MBDvtxZ_MBDchargeNorth;MBD vtxZ [cm];MBD charge sum (north)",120,-60,60,100,0,400)));
    ...
    h2D_map.insert(std::make_pair("h2D_NClus_ClusPhiSize", new TH2D("h2D_NClus_ClusPhiSize","h2D_NClus_ClusPhiSize;NClus;Cluster #phi size",100,0,2500,120,0,120)));

    // Division: --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    h1D_BestPair_DeltaEta = new TH1D("h1D_BestPair_DeltaEta","h1D_BestPair_DeltaEta;Best Pair #Delta#eta;Entries",200,-2,2);
    h1D_BestPair_DeltaPhi = new TH1D("h1D_BestPair_DeltaPhi","h1D_BestPair_DeltaPhi;Best Pair #Delta#phi;Entries",nDeltaPhiBin,DeltaPhiEdge_min,DeltaPhiEdge_max);

    h1D_BestPair_ClusEta_inner = new TH1D("h1D_BestPair_ClusEta_inner","h1D_BestPair_ClusEta_inner;ClusEta (inner);Entries",60,-3,3);
    h1D_BestPair_ClusEta_outer = new TH1D("h1D_BestPair_ClusEta_outer","h1D_BestPair_ClusEta_outer;ClusEta (outer);Entries",60,-3,3);

    h1D_BestPair_ClusAdc_inner = new TH1D("h1D_BestPair_ClusAdc_inner","h1D_BestPair_ClusAdc_inner;ClusAdc (inner);Entries",100,0,20000);
    h1D_BestPair_ClusAdc_outer = new TH1D("h1D_BestPair_ClusAdc_outer","h1D_BestPair_ClusAdc_outer;ClusAdc (outer);Entries",100,0,20000);
    h1D_BestPair_ClusAdc = new TH1D("h1D_BestPair_ClusAdc","h1D_BestPair_ClusAdc;ClusAdc (all);Entries",100,0,20000);

    h1D_BestPair_ClusPhiSize_inner = new TH1D("h1D_BestPair_ClusPhiSize_inner","h1D_BestPair_ClusPhiSize_inner;ClusPhiSize (inner);Entries",128,0,128);
    h1D_BestPair_ClusPhiSize_outer = new TH1D("h1D_BestPair_ClusPhiSize_outer","h1D_BestPair_ClusPhiSize_outer;ClusPhiSize (outer);Entries",128,0,128);
    h1D_BestPair_ClusPhiSize = new TH1D("h1D_BestPair_ClusPhiSize","h1D_BestPair_ClusPhiSize;ClusPhiSize (all);Entries",128,0,128);
    
    h2D_BestPair_ClusAdcClusPhiSize_inner = new TH2D("h2D_BestPair_ClusAdcClusPhiSize_inner","h2D_BestPair_ClusAdcClusPhiSize_inner;ClusAdc (inner);ClusPhiSize (inner)",100,0,20000,128,0,128);
    h2D_BestPair_ClusAdcClusPhiSize_outer = new TH2D("h2D_BestPair_ClusAdcClusPhiSize_outer","h2D_BestPair_ClusAdcClusPhiSize_outer;ClusAdc (outer);ClusPhiSize (outer)",100,0,20000,128,0,128);
    h2D_BestPair_ClusAdcClusPhiSize = new TH2D("h2D_BestPair_ClusAdcClusPhiSize","h2D_BestPair_ClusAdcClusPhiSize;ClusAdc (all);ClusPhiSize (all)",100,0,20000,128,0,128);

    h2D_BestPair_NClusClusAdc = new TH2D("h2D_BestPair_NClusClusAdc","h2D_BestPair_NClusClusAdc;NClus;ClusAdc (all)",200,0,2500,100,0,20000);
    h2D_BestPair_NClusClusPhiSize = new TH2D("h2D_BestPair_NClusClusPhiSize","h2D_BestPair_NClusClusPhiSize;NClus;ClusPhiSize (all)",200,0,2500,128,0,128);

    h2D_BestPair_ClusZClusAdc_inner = new TH2D("h2D_BestPair_ClusZClusAdc_inner","h2D_BestPair_ClusZClusAdc_inner;ClusPosZ [cm];ClusAdc (inner)",100,-30,30,100,0,20000);
    h2D_BestPair_ClusZClusAdc_outer = new TH2D("h2D_BestPair_ClusZClusAdc_outer","h2D_BestPair_ClusZClusAdc_outer;ClusPosZ [cm];ClusAdc (outer)",100,-30,30,100,0,20000);
    h2D_BestPair_ClusZClusAdc = new TH2D("h2D_BestPair_ClusZClusAdc","h2D_BestPair_ClusZClusAdc;ClusPosZ [cm];ClusAdc (all)",100,-30,30,100,0,20000);

    h2D_BestPair_ClusZClusPhiSize_inner = new TH2D("h2D_BestPair_ClusZClusPhiSize_inner","h2D_BestPair_ClusZClusPhiSize_inner;ClusPosZ [cm];ClusPhiSize (inner)",100,-30,30,128,0,128);
    h2D_BestPair_ClusZClusPhiSize_outer = new TH2D("h2D_BestPair_ClusZClusPhiSize_outer","h2D_BestPair_ClusZClusPhiSize_outer;ClusPosZ [cm];ClusPhiSize (outer)",100,-30,30,128,0,128);
    h2D_BestPair_ClusZClusPhiSize = new TH2D("h2D_BestPair_ClusZClusPhiSize","h2D_BestPair_ClusZClusPhiSize;ClusPosZ [cm];ClusPhiSize (all)",100,-30,30,128,0,128);

    h1D_BestPair_Vertical_ClusAdc_inner = new TH1D("h1D_BestPair_Vertical_ClusAdc_inner","h1D_BestPair_Vertical_ClusAdc_inner;ClusAdc (inner);Entries",100,0,20000);
    h1D_BestPair_Vertical_ClusAdc_outer = new TH1D("h1D_BestPair_Vertical_ClusAdc_outer","h1D_BestPair_Vertical_ClusAdc_outer;ClusAdc (outer);Entries",100,0,20000);
    h1D_BestPair_Vertical_ClusAdc = new TH1D("h1D_BestPair_Vertical_ClusAdc","h1D_BestPair_Vertical_ClusAdc;ClusAdc (all);Entries",100,0,20000);

    h1D_BestPair_Vertical_ClusPhiSize_inner = new TH1D("h1D_BestPair_Vertical_ClusPhiSize_inner","h1D_BestPair_Vertical_ClusPhiSize_inner;ClusPhiSize (inner);Entries",128,0,128);
    h1D_BestPair_Vertical_ClusPhiSize_outer = new TH1D("h1D_BestPair_Vertical_ClusPhiSize_outer","h1D_BestPair_Vertical_ClusPhiSize_outer;ClusPhiSize (outer);Entries",128,0,128);
    h1D_BestPair_Vertical_ClusPhiSize = new TH1D("h1D_BestPair_Vertical_ClusPhiSize","h1D_BestPair_Vertical_ClusPhiSize;ClusPhiSize (all);Entries",128,0,128);
}
...
void RestDist::PrepareEvent()
{
    for (int i = 0; i < run_nEvents; i++)
    {
                ...
                for (int scan_i = -5; scan_i < 6; scan_i++)
                {
                    ...
                    {                        
                        ...
                        if (isCheckBestPair) {
                            ...
                            temp_all_pair_DeltaPhi_vec.push_back(fabs(deltaPhi));
                        }
        ...
        if (isCheckBestPair && MBD_centrality <= 70){
            long long vec_size = temp_all_pair_DeltaPhi_vec.size();
            long long ind[temp_all_pair_DeltaPhi_vec.size()];
            TMath::Sort(vec_size, &temp_all_pair_DeltaPhi_vec[0], ind, false);    
            ...
            for (int pair_i = 0; pair_i < vec_size; pair_i++){
                ...
                if (fabs(pair.delta_phi) > 0.026) {break;}

                if (i%50 == 0){
                    std::cout << "Best Pair " << i << " : " << pair_i << std::endl;
                    std::cout << "Delta Eta = " << pair.delta_eta << ", Delta Phi = " << pair.delta_phi << std::endl;
                    std::cout << "Inner Clu Index = " << pair.inner_index << ", Outer Clu Index = " << pair.outer_index << std::endl;
                    std::cout << "Rough confirmation, DeltaEta: "<<ClusEta_INTTz->at(pair.inner_index) - ClusEta_INTTz->at(pair.outer_index) << std::endl;
                }

                h1D_BestPair_ClusEta_inner->Fill(ClusEta_INTTz->at(pair.inner_index), INTTvtxZWeighting);
                h1D_BestPair_ClusEta_outer->Fill(ClusEta_INTTz->at(pair.outer_index), INTTvtxZWeighting);

                h1D_BestPair_DeltaEta->Fill(pair.delta_eta, INTTvtxZWeighting);
                h1D_BestPair_DeltaPhi->Fill(pair.delta_phi, INTTvtxZWeighting);

                h1D_BestPair_ClusAdc_inner->Fill(ClusAdc->at(pair.inner_index), INTTvtxZWeighting);
                h1D_BestPair_ClusAdc_outer->Fill(ClusAdc->at(pair.outer_index), INTTvtxZWeighting);
                h1D_BestPair_ClusAdc->Fill(ClusAdc->at(pair.inner_index), INTTvtxZWeighting);
                h1D_BestPair_ClusAdc->Fill(ClusAdc->at(pair.outer_index), INTTvtxZWeighting);

                h1D_BestPair_ClusPhiSize_inner->Fill(ClusPhiSize->at(pair.inner_index), INTTvtxZWeighting);
                h1D_BestPair_ClusPhiSize_outer->Fill(ClusPhiSize->at(pair.outer_index), INTTvtxZWeighting);
                h1D_BestPair_ClusPhiSize->Fill(ClusPhiSize->at(pair.inner_index), INTTvtxZWeighting);
                h1D_BestPair_ClusPhiSize->Fill(ClusPhiSize->at(pair.outer_index), INTTvtxZWeighting);
                
                h2D_BestPair_ClusAdcClusPhiSize_inner->Fill(ClusAdc->at(pair.inner_index), ClusPhiSize->at(pair.inner_index), INTTvtxZWeighting);
                h2D_BestPair_ClusAdcClusPhiSize_outer->Fill(ClusAdc->at(pair.outer_index), ClusPhiSize->at(pair.outer_index), INTTvtxZWeighting);
                h2D_BestPair_ClusAdcClusPhiSize->Fill(ClusAdc->at(pair.inner_index), ClusPhiSize->at(pair.inner_index), INTTvtxZWeighting);
                h2D_BestPair_ClusAdcClusPhiSize->Fill(ClusAdc->at(pair.outer_index), ClusPhiSize->at(pair.outer_index), INTTvtxZWeighting);
                
                h2D_BestPair_NClusClusAdc->Fill(total_NClus, ClusAdc->at(pair.inner_index), INTTvtxZWeighting);
                h2D_BestPair_NClusClusAdc->Fill(total_NClus, ClusAdc->at(pair.outer_index), INTTvtxZWeighting);
                h2D_BestPair_NClusClusPhiSize->Fill(total_NClus, ClusPhiSize->at(pair.inner_index), INTTvtxZWeighting);
                h2D_BestPair_NClusClusPhiSize->Fill(total_NClus, ClusPhiSize->at(pair.outer_index), INTTvtxZWeighting);
                
                h2D_BestPair_ClusZClusAdc_inner->Fill(ClusZ->at(pair.inner_index), ClusAdc->at(pair.inner_index), INTTvtxZWeighting);
                h2D_BestPair_ClusZClusAdc_outer->Fill(ClusZ->at(pair.outer_index), ClusAdc->at(pair.outer_index), INTTvtxZWeighting);
                h2D_BestPair_ClusZClusAdc->Fill(ClusZ->at(pair.inner_index), ClusAdc->at(pair.inner_index), INTTvtxZWeighting);
                h2D_BestPair_ClusZClusAdc->Fill(ClusZ->at(pair.outer_index), ClusAdc->at(pair.outer_index), INTTvtxZWeighting);
                
                h2D_BestPair_ClusZClusPhiSize_inner->Fill(ClusZ->at(pair.inner_index), ClusPhiSize->at(pair.inner_index), INTTvtxZWeighting);
                h2D_BestPair_ClusZClusPhiSize_outer->Fill(ClusZ->at(pair.outer_index), ClusPhiSize->at(pair.outer_index), INTTvtxZWeighting);
                h2D_BestPair_ClusZClusPhiSize->Fill(ClusZ->at(pair.inner_index), ClusPhiSize->at(pair.inner_index), INTTvtxZWeighting);
                h2D_BestPair_ClusZClusPhiSize->Fill(ClusZ->at(pair.outer_index), ClusPhiSize->at(pair.outer_index), INTTvtxZWeighting);

                if (fabs(ClusEta_INTTz->at(pair.inner_index))<0.1){
                    h1D_BestPair_Vertical_ClusAdc_inner->Fill(ClusAdc->at(pair.inner_index), INTTvtxZWeighting);
                    h1D_BestPair_Vertical_ClusAdc->Fill(ClusAdc->at(pair.inner_index), INTTvtxZWeighting);
                    
                    h1D_BestPair_Vertical_ClusPhiSize_inner->Fill(ClusPhiSize->at(pair.inner_index), INTTvtxZWeighting);
                    h1D_BestPair_Vertical_ClusPhiSize->Fill(ClusPhiSize->at(pair.inner_index), INTTvtxZWeighting);
                }

                if (fabs(ClusEta_INTTz->at(pair.outer_index))<0.1){
                    h1D_BestPair_Vertical_ClusAdc_outer->Fill(ClusAdc->at(pair.outer_index), INTTvtxZWeighting);
                    h1D_BestPair_Vertical_ClusAdc->Fill(ClusAdc->at(pair.outer_index), INTTvtxZWeighting);
                    
                    h1D_BestPair_Vertical_ClusPhiSize_outer->Fill(ClusPhiSize->at(pair.outer_index), INTTvtxZWeighting);
                    h1D_BestPair_Vertical_ClusPhiSize->Fill(ClusPhiSize->at(pair.outer_index), INTTvtxZWeighting);
                }
...
void RestDist::EndRun()
{
    ...
    if (isCheckBestPair){
        h1D_BestPair_DeltaEta -> Write();
        h1D_BestPair_DeltaPhi -> Write();

        h1D_BestPair_ClusEta_inner -> Write();
        h1D_BestPair_ClusEta_outer -> Write();
        
        h1D_BestPair_ClusAdc_inner -> Write();
        h1D_BestPair_ClusAdc_outer -> Write();
        h1D_BestPair_ClusAdc -> Write();

        h1D_BestPair_ClusPhiSize_inner -> Write();
        h1D_BestPair_ClusPhiSize_outer -> Write();
        h1D_BestPair_ClusPhiSize -> Write();

        h2D_BestPair_ClusAdcClusPhiSize_inner -> Write();
        h2D_BestPair_ClusAdcClusPhiSize_outer -> Write();
        h2D_BestPair_ClusAdcClusPhiSize -> Write();

        h2D_BestPair_NClusClusAdc -> Write();
        h2D_BestPair_NClusClusPhiSize -> Write();
        
        h2D_BestPair_ClusZClusAdc_inner -> Write();
        h2D_BestPair_ClusZClusAdc_outer -> Write();
        h2D_BestPair_ClusZClusAdc -> Write();
        
        h2D_BestPair_ClusZClusPhiSize_inner -> Write();
        h2D_BestPair_ClusZClusPhiSize_outer -> Write();
        h2D_BestPair_ClusZClusPhiSize -> Write();

        h1D_BestPair_Vertical_ClusAdc_inner -> Write();
        h1D_BestPair_Vertical_ClusAdc_outer -> Write();
        h1D_BestPair_Vertical_ClusAdc -> Write();

        h1D_BestPair_Vertical_ClusPhiSize_inner -> Write();
        h1D_BestPair_Vertical_ClusPhiSize_outer -> Write();
        h1D_BestPair_Vertical_ClusPhiSize -> Write();
    }
...
std::pair<double,double> RestDist::Get_possible_zvtx(double rvtx, std::vector<double> p0, std::vector<double> p1) // note : inner p0, outer p1, vector {r,z, zid}, -> {y,x}
{
...
double RestDist::Get_extrapolation(double given_y, double p0x, double p0y, double p1x, double p1y) // note : x : z, y : r
{
...
double RestDist::get_radius(double x, double y)
{
