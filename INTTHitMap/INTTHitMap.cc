...

{
  ...
  if (trigger_MBDvtxZ_cm != 10 && trigger_MBDvtxZ_cm != 150) {
    std::cout<<"INTTHitMap::INTTHitMap - trigger_MBDvtxZ_cm is not 10 or 150, please check the input"<<std::endl;
    exit(1);
  }
...
int INTTHitMap::process_event(PHCompositeNode *topNode)
{
  ...
  if (MBDNS_trigger_require_tag)
  {
    ...
    if (trigger_MBDvtxZ_cm == 150 && live_trigger_map.find(MBDNS_VtxZ150cm_Id) == live_trigger_map.end()){
      eID_count++;
      return Fun4AllReturnCodes::EVENT_OK;
    }
  ...
  for (int i1 = 0; i1 < 8; i1++){
    inttcont = nullptr;  
    inttcont = findNode::getClass<InttRawHitContainer>(topNode, Form("INTTRAWHIT_%d",i1));

    if (!inttcont)
    {
      std::cout << PHWHERE << std::endl;
      std::cout << "InttCombinedRawDataDecoder::process_event(PHCompositeNode* topNode)" << std::endl;
      std::cout << "Could not get \" INTTRAWHIT_" << i1 << "\" from Node Tree" << std::endl;
    ...
    if (inttcont->get_nhits() == 0) {
      std::cout << "eID: "<< eID_count <<" INTTBcoResolution::PrepareINTT - no INTT hit found, INTTRAWHIT_" << i1 << std::endl;
    ...
    for (unsigned int i = 0; i < inttcont->get_nhits(); i++)
    {
      ...
      // if (raw.felix_server == 2 && raw.felix_channel == 9){
      //   std::cout << raw.felix_server << " " << raw.felix_channel << " " << raw.chip << " " << raw.channel <<" " << bco_diff << "\n"<<std::endl;
      // }
      
      // if (raw.felix_server == 2 && raw.felix_channel == 9 && raw.chip == 15){
      //   continue;
      // }
