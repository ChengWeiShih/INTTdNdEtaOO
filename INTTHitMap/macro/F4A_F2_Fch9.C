...
void F4A_F2_Fch9(
  ...
  int run_num = 54966,
  int nevents = 1000,
  string output_directory = "/sphenix/tg/tg01/commissioning/INTT/work/cwshih/INTT_F2_Fch9_study/Run24_AuAu_54966",
  string input_directory = "I_am_cool", //"/sphenix/lustre01/sphnxpro/production/run3oo/physics/ana537_nocdbtag_v001",
  string input_filename = "I_am_cool",
  
  // todo : modify here
  string hot_channel_full_file_directory = "/cvmfs/sphenix.sdcc.bnl.gov/calibrations/sphnxpro/cdb/INTT_HotMap/18/83/1883b7abd5223296e7259fe3b88c5f73_hotmap_run_00054966.root",
  // string hot_channel_full_file_directory = "/sphenix/user/jaein213/calibrationcheck/hotmap/CALIB_INTT_HOTMAP_MERGED-00072151.root",
  int bco_diff_peak = 49,
  
  bool apply_hot_channel = true,
  bool apply_bco_diff = false,
  bool hitQA_check = true,
  bool clone_hit_remove_BCO = true,
  bool MBDNS_trigger_require_tag = false,
  int  trigger_MBDvtxZ_cm = 10 // note : cm
...
)
{
  ...
  for (int i = 0; i < 8; i++)
  {
      ...
      std::string intt_file_name = Form("DST_STREAMING_EVENT_INTT%d_run2auau_ana464_nocdbtag_v001-000%d-%s.root", i, run_num, job_index.c_str());
      // std::string intt_file_name = Form("DST_STREAMING_EVENT_intt%d_run3auau_new_nocdbtag_v001-000%d-%s.root", i, run_num, job_index.c_str());
