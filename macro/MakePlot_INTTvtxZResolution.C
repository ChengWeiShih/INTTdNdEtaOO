...
int MakePlot_INTTvtxZResolution()
{
    ...
    std::string target_plot_name = "h1D_INTTvtxZ_resolution_Inclusive70";
    std::string output_directory = input_file_directory;
    std::string output_file_name = "h1D_INTTvtxZ_resolution_Inclusive70";
    // std::pair<std::string, std::string> axes_label = {"INTT LayerID", "INTT LadderPhiId"};
    std::string Zaxis_title = "";
    std::pair<std::string, std::string> axes_label = {"INTT-standalone z-vertex - Truth [cm]", "Counts"};
    ...
    std::vector<std::tuple<double,double,std::string>> additional_text = {
        ...
        {0.2, 0.88, "HIJING O+O 200 GeV"},
        // {0.22, 0.825, "HIJING"},   // todo: ----------> Change this one if needed
        // {0.22, 0.82, "NClus > 500"},
        {0.2, 0.825, "Centrality 0-70%"},
    ...
    if (isTH1D){
        ...
        if (DoGausFit){
            // leg -> AddEntry(h1D_in, "INTT-only z-vertex diff.", "l");
            leg -> AddEntry(gaus_fit, "Gaussian Fit", "l");
    ...
    std::string final_label = (sPH_label.size() == 0) ? Form("#it{#bf{sPHENIX}} Simulation") : Form("#it{#bf{sPHENIX}} Simulation %s", sPH_label.c_str());
    ...
    if (DoGausFit)
    {
        draw_text -> DrawLatex(
            ...
            Form("Mean_{Gaus. Fit} = %.3f cm", gaus_fit -> GetParameter(1))
        );

        draw_text -> DrawLatex(
            std::get<0>(additional_text[0]),
            std::get<1>(additional_text.back()) - 0.14,
            Form("#sigma_{Gaus. Fit} = %.2f cm", gaus_fit -> GetParameter(2))
        );
    }

    c1 -> Print(Form("%s/%s_%s.pdf", output_directory.c_str(), output_file_name.c_str(),sPH_label.c_str()));
    c1 -> Clear();
