...
namespace
{
    ...
    constexpr const char* kHistName = "h1D_GoodProtoTracklet_Eta_Subtracted_normalized";
...
int PlotBunchNumberEtaZeroDependenceImpl()
{
    ...
    graphs.front()->SetTitle("Intt Doublets per evt. per #eta (|#eta|<0.5 && |#Delta#phi|<0.15)");
    graphs.front()->GetXaxis()->SetTitle("Average FPHX BCO (Bunch number range)");
    graphs.front()->GetYaxis()->SetTitle("Intt Doublets (per evt. per #eta)");
    ...
    const TString outputPdf = Form("%s/BunchNumber_eta0_dependence.pdf", kMotherDir);
...
void PlotBunchNumberEtaZeroDependence()
{
