...
void InttDoubletMap(TString inputPath = "/sphenix/tg/tg01/commissioning/INTT/work/cwshih/Run25/dNdEtaOO/CarryOverHit/82420/InttDoublets/FPHX_range_10/completed/Data_INTTDoublets_StreamTrig_FphxBcoRange106to116_VtxZRangeN15to15_ClusQAAdc0PhiSize128_DeltaPhiCut0p150_00082420_merged.root")
{
    ...
    TString outputPath = Form("%s/%s_output.root", inputDir.Data(), inputBase.Data());
    ...
    // --- 2D histograms ---
    TH2D *h2D_EtaPhi         = (TH2D*)fin->Get("h2D_GoodProtoTracklet_EtaPhi");
    TH2D *h2D_EtaPhi_rot     = (TH2D*)fin->Get("h2D_GoodProtoTracklet_EtaPhi_rotated");
    TH2D *h2D_ClusMap        = (TH2D*)fin->Get("h2D_Clus_ColumnZID_LayerPhiID");

    // --- 1D histograms ---
    TH1D *h1D_Eta            = (TH1D*)fin->Get("h1D_GoodProtoTracklet_Eta");
    TH1D *h1D_Eta_rot        = (TH1D*)fin->Get("h1D_GoodProtoTracklet_Eta_rotated");
    ...
    TH1D *h1D_TruthHadron      = (TH1D*)fin->Get("h1D_Truth_ChargedHadron");
    TH2D *h2D_TruthEtaPhi      = (TH2D*)fin->Get("h2D_Truth_ChargedHadron_EtaPhi");
    TH1D *h1D_TruthHadronEta   = (TH1D*)fin->Get("h1D_Truth_ChargedHadron_Eta");

    // Basic sanity checks on required histograms
    if (!h2D_EtaPhi || !h2D_EtaPhi_rot) { std::cout << "Error: Missing h2D_GoodProtoTracklet_EtaPhi histograms" << std::endl; return; }
    if (!h1D_Eta    || !h1D_Eta_rot)     { std::cout << "Error: Missing h1D_GoodProtoTracklet_Eta histograms" << std::endl; return; }
    ...
    if (!h2D_ClusMap)                     { std::cout << "Error: Missing h2D_Clus_ColumnZID_LayerPhiID" << std::endl; return; }
    ...
    // =====================================================================
    TH2D *h2D_EtaPhi_sub = (TH2D*)h2D_EtaPhi->Clone("h2D_GoodProtoTracklet_EtaPhi_Subtracted");
    h2D_EtaPhi_sub->SetTitle("h2D_GoodProtoTracklet_EtaPhi - rotated;Pair #eta;Phi [radian]");
    h2D_EtaPhi_sub->Add(h2D_EtaPhi_rot, -1.0);

    // =====================================================================
    // Task 2: h1D subtraction  Eta - Eta_rotated
    // =====================================================================
    TH1D *h1D_Eta_sub = (TH1D*)h1D_Eta->Clone("h1D_GoodProtoTracklet_Eta_Subtracted");
    h1D_Eta_sub->SetTitle("h1D_GoodProtoTracklet_Eta - rotated;#eta;Entries");
    h1D_Eta_sub->Add(h1D_Eta_rot, -1.0);

    // =====================================================================
    // Task 6: Normalized dN/deta  = (Eta_sub) / binWidth / nEvents
    // =====================================================================
    TH1D *h1D_GoodProtoTracklet_Eta_Subtracted_normalized = (TH1D*)h1D_Eta_sub->Clone("h1D_GoodProtoTracklet_Eta_Subtracted_normalized");
    h1D_GoodProtoTracklet_Eta_Subtracted_normalized->SetTitle("h1D_GoodProtoTracklet_Eta_Subtracted_normalized;#eta;INTT doublets (normalized)");
    ...
    TH1D *h1D_IntegralEta = (TH1D*)h1D_Eta->Clone("h1D_DeltaPhi_IntegralEta");
    h1D_IntegralEta->Reset();
    h1D_IntegralEta->SetTitle(Form("DeltaPhi integral [%.2f, %.2f] vs #eta;#eta;Integral", DeltaPhi_cut.first, DeltaPhi_cut.second));

    int nEtaBin_DeltaPhi = 27; // 0..26
    for (int eta = 0; eta < nEtaBin_DeltaPhi; eta++)
    {
        ...
        if (eta + 1 <= h1D_IntegralEta->GetNbinsX()) {
            h1D_IntegralEta->SetBinContent(eta + 1, integral);
        }
    }
    ...
    // =====================================================================
    TH2D *h2D_IntegralEtaPhi = (TH2D*)h2D_EtaPhi->Clone("h2D_DeltaPhi_IntegralEtaPhi");
    ...
    // =====================================================================
    TH1D *h1D_TruedNdEta = nullptr;
    if (h1D_TruthHadronEta) {
        h1D_TruedNdEta = (TH1D*)h1D_TruthHadronEta->Clone("h1D_Truth_ChargedHadron_dNdEta");
        h1D_TruedNdEta->SetTitle("True dN/d#eta;#eta;dN/d#eta");
        for (int i = 1; i <= h1D_TruedNdEta->GetNbinsX(); i++) {
            double binW = h1D_TruedNdEta->GetBinWidth(i);
            double val  = h1D_TruedNdEta->GetBinContent(i);
            double err  = h1D_TruedNdEta->GetBinError(i);
            h1D_TruedNdEta->SetBinContent(i, val / binW / nEvents);
            h1D_TruedNdEta->SetBinError(i, err / binW / nEvents);
        }
    }
    ...
    // Task 8 summary
    h1D_IntegralEta->Write();

    // Task 1
    h2D_EtaPhi_sub->Write();
    ...
    if (h1D_TruthHadron)    h1D_TruthHadron->Write();
    if (h2D_TruthEtaPhi)    h2D_TruthEtaPhi->Write();
    if (h1D_TruthHadronEta) h1D_TruthHadronEta->Write();

    // Task 13: Normalized truth dN/deta
    if (h1D_TruedNdEta) h1D_TruedNdEta->Write();
