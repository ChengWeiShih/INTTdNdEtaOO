...
int INTTRawHit_check_BarrelNIM_CentralityTest(
    int process_id = 0,
    int run_num = 82525,
    int nevents = 20,
    string output_directory = "/sphenix/tg/tg01/commissioning/INTT/work/cwshih/Run25/dNdEtaOO/82525/TrigCrossingClus_AlignP",
    ...
    string output_file_name_suffix_in = "ana551",

    std::pair<bool, std::string> PrivateCentrality = {true, "/sphenix/user/ChengWei/INTT/INTTdNdEtaOO/SelfProduction/GlauberTest/run82525/GlauberNBDfit_centrality_bounds_1pct.txt"},
    ...
    std::tuple<bool, std::string, std::string, int> PrivateCentralityFit = {
        ...
        "/sphenix/user/ChengWei/INTT/INTTdNdEtaOO/SelfProduction/GlauberTest/run82525/GlauberNBDfit_output.root",
    ...
    string hotchannel_full_directory_in = Form("/cvmfs/sphenix.sdcc.bnl.gov/calibrations/sphnxpro/cdb/INTT_HotMap/a6/06/a606cb5636c9a2ebd5c30ce4ded6fcdd_intt_hotmap_cdb_00082525.root"); // note : this is for the DAC Scan runs
    // string bco_full_directory_in        =      "/cvmfs/sphenix.sdcc.bnl.gov/calibrations/sphnxpro/cdb/INTT_BCOMAP/06/05/06051ec973b8e40ac9848ebb76237811_intt_bcomap_cdb_82391.root";
    // int triggered_BcoDiff_in = GetTriggeredPeak(bco_full_directory_in);
    int triggered_BcoDiff_in = 50;
    ...
    //     {82391, "/sphenix/user/ChengWei/SelfINTTProduction/2025_Oct_20/Analysis/ProductionPreparation/cdb_intt_dac_30_35_45_80_115_150_185_215.root"}
    ...
    std::string job_index_over10 = std::to_string( process_id/10 );
    job_index_over10.insert(0, job_index_len - job_index_over10.size(), '0');

    string output_DST_name = Form("DST_test_000%d_standalone_%s_%s.root", run_num, output_file_name_suffix_in.c_str(), job_index.c_str());
    string output_ntuple_name = Form("Ntuple_000%d_standalone_%s_%s.root", run_num, output_file_name_suffix_in.c_str(), job_index.c_str());


    Fun4AllInputManager *in_calo = new Fun4AllDstInputManager("DST_calo");
    in_calo->Verbosity(2);
    in_calo->AddFile( Form("/sphenix/lustre01/sphnxpro/production/run3oo/physics/pro001_pcdb001_v001/DST_CALOFITTING/run_00082500_00082600/DST_CALOFITTING_run3oo_pro001_pcdb001_v001-00082525-%s.root", job_index_over10.c_str()) );
    in_calo->Print("ALL");
    ...
    for (int i = 0; i < 8; i++)
    {
        std::string intt_folder_direcotry = Form("DST_STREAMING_EVENT_intt%d/run_00082500_00082600", i);
    ...
    for (int i = 0; i < 8; i++){
        ...
        myDecoder->runInttStandalone(false);
        ...
        myDecoder->set_triggeredMode(false);
        myDecoder->set_bcoFilter(false);
    ...
    // mb->Verbosity(11110); 
    mb->setSpecies(MinimumBiasInfo::SPECIES::OO);
    // mb->setOverwriteScale("/sphenix/user/ChengWei/INTT/INTTdNdEtaOO/SelfProduction/PrivateCentralityScaleFile/cdb_centrality_scale_82391_A.root");
    se->registerSubsystem(mb);

    CentralityReco *cr = new CentralityReco();
    // cr->Verbosity(11110);
    // cr->setOverwriteScale("/sphenix/user/ChengWei/INTT/INTTdNdEtaOO/SelfProduction/PrivateCentralityScaleFile/cdb_centrality_scale_82391_A.root");
    se->registerSubsystem(cr);
