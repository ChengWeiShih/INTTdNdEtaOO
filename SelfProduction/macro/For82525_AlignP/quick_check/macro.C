...
int macro(
    ...
    string output_directory = "/sphenix/tg/tg01/commissioning/INTT/work/cwshih/Run25/dNdEtaOO/82525/ForCentrality/quick_hist";
    ...
    TFile * file_in = TFile::Open(Form("/sphenix/tg/tg01/commissioning/INTT/work/cwshih/Run25/dNdEtaOO/82525/ForCentrality/completed/Ntuple_00082525_standalone_ana551_%s.root",job_index.c_str()));
    ...
    TH1D * h1D_MinBias_MBD_charge_sum = new TH1D("h1D_MinBias_MBD_charge_sum","h1D_MinBias_MBD_charge_sum;MBD charge sum;Counts",400,0,400);
    TH2D * h2D_MinBias_INTTNClus_MBDChargeSum = new TH2D("h2D_MinBias_INTTNClus_MBDChargeSum","h2D_MinBias_INTTNClus_MBDChargeSum;# of INTT clusters;MBD charge sum",400,0,2000,400,0,400);
    TH2D * h2D_MinBias_MBDCharge_North_South = new TH2D("h2D_MinBias_MBDCharge_North_South","h2D_MinBias_MBDCharge_North_South;MBD charge (north);MBD charge (south)",200,0,200,200,0,200);
    TH1D * h1D_MinBias_MBD_z_vtx = new TH1D("h1D_MinBias_MBD_z_vtx","h1D_MinBias_MBD_z_vtx;MBD z-vertex [cm];Counts",200,-100,100);
    TH2D * h2D_MinBias_MBDz_MBDChargeAsym = new TH2D("h2D_MinBias_MBDz_MBDChargeAsym","h2D_MinBias_MBDz_MBDChargeAsym;MBD z-vertex [cm];MBD charge asymmetry",200,-100,100,200,-2,2);
    TH1D * h1D_MinBias_centrality = new TH1D("h1D_MinBias_centrality","h1D_MinBias_centrality; Event Centrality;Counts",101,-0.5,100.5);
    TH1D * h1D_MinBias_centrality_2 = new TH1D("h1D_MinBias_centrality_2","h1D_MinBias_centrality_2; Event Centrality;Counts",101,-0.5,100.5);
    TH1D * h1D_MinBias_centrality_3 = new TH1D("h1D_MinBias_centrality_3","h1D_MinBias_centrality_3; Event Centrality;Counts",101,-0.5,100.5);
    ...
    for (int i = 0; i < tree -> GetEntries(); i++){
        ...
        h1D_MinBias_MBD_charge_sum -> Fill(MBD_charge_sum);
        
        h2D_MinBias_MBDCharge_North_South -> Fill(MBD_north_charge_sum,MBD_south_charge_sum);
        h1D_MinBias_MBD_z_vtx -> Fill(MBD_z_vtx);
        h2D_MinBias_MBDz_MBDChargeAsym -> Fill(MBD_z_vtx,MBD_charge_asymm);

        h1D_MinBias_centrality -> Fill(
            GetCentralityBin_private(MBD_charge_sum)
        );

        h1D_MinBias_centrality_3 -> Fill(
            int(100. * (f_NBDGlauber->Integral(MBD_charge_sum,400)/f_NBDGlauber->Integral(0,400))) + 1
        );
        
        if (fabs(MBD_z_vtx) < 10){
            h2D_MinBias_INTTNClus_MBDChargeSum -> Fill(NClus, MBD_charge_sum);
        }
    }
    ...
    h1D_MinBias_MBD_charge_sum -> Write();
    h2D_MinBias_INTTNClus_MBDChargeSum -> Write();
    h2D_MinBias_MBDCharge_North_South -> Write();
    h1D_MinBias_MBD_z_vtx -> Write();
    h2D_MinBias_MBDz_MBDChargeAsym -> Write();
    h1D_MinBias_centrality -> Write();
    h1D_MinBias_centrality_2 -> Write();
    h1D_MinBias_centrality_3 -> Write();
