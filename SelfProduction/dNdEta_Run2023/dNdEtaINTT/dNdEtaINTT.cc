...
int dNdEtaINTT::Init(PHCompositeNode *topNode)
{
    ...
    if (_get_intt_data)
    {
        ...
        if (_get_reco_cluster)
        {
            ...
            outtree->Branch("ClusLocalX", &ClusLocalX_);
            outtree->Branch("ClusLocalY", &ClusLocalY_);
        ...
        if (_get_trigger_info)
        {
            ...
            outtree->Branch("BunchNumber", &BunchNumber_);
        }

        if (_get_trigger_fire)
        {
            outtree->Branch("MBDNS1_live_inclusive", &out_MBDNS1_live_inclusive);
            outtree->Branch("MBDNS1_live_vtx10cm", &out_MBDNS1_live_vtx10cm);
            outtree->Branch("MBDNS1_scaled_vtx10cm", &out_MBDNS1_scaled_vtx10cm);
            outtree->Branch("MBDNS1_scaled_vtx150cm", &out_MBDNS1_scaled_vtx150cm);
        }
    }
...
void dNdEtaINTT::GetTriggerInfo(PHCompositeNode *topNode)
{
    ...
    BunchNumber_ = gl1packet->getBunchNumber();
...
void dNdEtaINTT::GetTriggerFireInfo(PHCompositeNode *topNode)
{
    std::cout << "Get GL1 trigger info. for getting trigger fire" << std::endl;

    long long live_trigger_decimal = -999;
    long long scaled_trigger_decimal = -999;

    out_MBDNS1_live_inclusive = -999;
    out_MBDNS1_live_vtx10cm = -999;
    out_MBDNS1_scaled_vtx10cm = -999;
    out_MBDNS1_scaled_vtx150cm = -999;


    live_trigger_decimal   = gl1packet->lValue(0,"LiveVector");
    scaled_trigger_decimal = gl1packet->lValue(0,"ScaledVector");

    if (live_trigger_decimal < 0 || scaled_trigger_decimal < 0)
    {
        std::cout << "INTTRawHitSanityCheck::PrepareGL1 - live or scaled trigger is negative, "<< live_trigger_decimal << ", "<< scaled_trigger_decimal << std::endl;
        // return Fun4AllReturnCodes::EVENT_OK;
    }

         
    std::map<int,int> live_trigger_map   = prepare_trigger_map(prepare_trigger_vec(live_trigger_decimal));
    std::map<int,int> scaled_trigger_map = prepare_trigger_map(prepare_trigger_vec(scaled_trigger_decimal));
    ...
    out_MBDNS1_live_inclusive  = (live_trigger_map.find(MBDNS1_inclusive) != live_trigger_map.end()) ? 1 : 0;
    out_MBDNS1_live_vtx10cm    = (live_trigger_map.find(MBDNS1_vtx10cm) != live_trigger_map.end()) ? 1 : 0;
    out_MBDNS1_scaled_vtx10cm  = (scaled_trigger_map.find(MBDNS1_vtx10cm) != scaled_trigger_map.end()) ? 1 : 0;
    out_MBDNS1_scaled_vtx150cm = (scaled_trigger_map.find(MBDNS1_vtx150cm) != scaled_trigger_map.end()) ? 1 : 0;
...
void dNdEtaINTT::GetInttRawHitInfo(PHCompositeNode *topNode)
{
    ...
    NInttRawHits_ = 0;
    for (int j = 0; j < 8; j++){
        inttrawhitcontainer = findNode::getClass<InttRawHitContainer>(topNode, Form("INTTRAWHIT_%d",j));
        
        if (!inttrawhitcontainer)
        {
            std::cout << PHWHERE << "Error, can't find INTTRAWHIT_" <<j<< std::endl;
            exit(1);
        }
...
void dNdEtaINTT::GetRecoClusterInfo(PHCompositeNode *topNode)
{
    ...
    for (const auto &hitsetkey : dst_clustermap->getHitSetKeys(TrkrDefs::inttId))
    {
        ...
        for (auto iter = range.first; iter != range.second; ++iter)
        {
            ...
            ClusLocalX_.push_back(cluster->getLocalX());
            ClusLocalY_.push_back(cluster->getLocalY());
...
void dNdEtaINTT::GetPHG4Info(PHCompositeNode *topNode)
{
    ...
    std::cout<<"Number of primary G4Particles: " << NPrimaryG4P_ << std::endl;
    std::cout<<"Number of primary charged hadrons: " << NPrimaryG4P_promptChargeHadron_ << std::endl;
...
void dNdEtaINTT::ResetVectors()
{
    ...
    CleanVec(ClusPhi_);
    CleanVec(ClusEta_);
    CleanVec(ClusLocalX_);
    CleanVec(ClusLocalY_);
    CleanVec(ClusAdc_);
...
std::vector<int> dNdEtaINTT::prepare_trigger_vec(long long trigger_input)
{
...
std::map<int,int> dNdEtaINTT::prepare_trigger_map(std::vector<int> trigger_vec_input)
{
